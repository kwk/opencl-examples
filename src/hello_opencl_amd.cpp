...
int main() {
    std::cout << "OpenCL Hello World - AMD GPU Device" << std::endl;
    ...
    for (cl_uint i = 0; i < numPlatforms; i++) {
        ...
        if (std::strstr(platformName, "Advanced Micro Devices") != NULL ||
            std::strstr(platformName, "AMD") != NULL) {
            platform = platforms[i];
            std::cout << "Found AMD platform: " << platformName << std::endl;
            break;
        }
    }

    if (platform == NULL) {
        std::cerr << "AMD platform not found" << std::endl;
        return 1;
    }
