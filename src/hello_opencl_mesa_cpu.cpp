...
int main() {
    std::cout << "OpenCL Hello World - Mesa CPU Device" << std::endl;
    ...
    if (platform == nullptr) {
        ...
        std::cerr << "\nNote: Mesa OpenCL (Rusticl/Clover) requires compatible hardware." << std::endl;
        return 1;
    }
    ...
    if (err != CL_SUCCESS) {
        std::cerr << "Failed to get CPU device from Mesa platform" << std::endl;
        std::cerr << "Note: Mesa typically provides GPU devices. For CPU OpenCL, use PoCL instead." << std::endl;
        return 1;
    }
